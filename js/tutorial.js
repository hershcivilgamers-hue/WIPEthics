...
const STEP_DEFS = [
  ...
  { hash: '#/omega-1',            get title() { return `${ORGS['omega-1'].short} Personnel`; },   get body() { return `${ORGS['omega-1'].name} personnel files: ranks, clearances, strikes, medals, tags and service history.`; } },
  { hash: '#/omega-1/recruitment', get title() { return `${ORGS['omega-1'].short} Recruitment`; },    get body() { return `Applications to ${ORGS['omega-1'].name}, with panel voting.`; }, minor: true },
